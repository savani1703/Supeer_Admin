...
namespace App\Models\PaymentManual\YES;
...
class YesPayoutMeta extends Model
{
    protected $connection = 'payment_manual';
    protected $table = 'tbl_yes_payout_meta';
    ...
    private $defaultAccountId = "YES000";
    private $defaultAccountIdPrefix = "YES";
    ...
    public function addMeta($formData, $accountId) {
        try {
            ...
            $this->merchant_id = PgName::IDFC.$accountId;
    ...
    {
        try {
            $result = $this->where('debit_account',$accountNumber)->first();
            if($result){
                return $result;
            }
            return null;
    ...
    public function getAccountNumberByEmailId($notifyEmailId)
    {
        try {
            $result = $this->where('notify_email_id', $notifyEmailId)->value('debit_account');
            if($result){
                return $result;
            }
            return null;
    ...
    public function getAccountNumberByCustomerId($customerId)
    {
        try {
            $result = $this->where('customer_id', $customerId)->value('debit_account');
            if($result){
                return $result;
            }
            return null;
