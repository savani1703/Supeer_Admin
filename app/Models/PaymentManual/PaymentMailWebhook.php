...
class PaymentMailWebhook extends Model
{
    protected $connection = 'payment_manual';
    protected $table = 'tbl_payment_mail_webhook';
    ...
    {
        try {
            $result = $this->where('payout_id', $payoutId)->where('account_number', $accountNumber)->value('bank_rrn');
            if($result){
                return $result;
            }
            return null;
