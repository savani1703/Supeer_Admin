...
namespace App\Models\PaymentAuto\Payg;
...
class PaygPayoutMeta extends Model
{
    use Encryptable;

    protected $connection = 'payment_auto';
    protected $table = 'tbl_payg_payout_meta';
    ...
    private $defaultAccountId = "PG000";
    private $defaultAccountIdPrefix = "PG";
    ...
    public function addMeta($formData, $accountId) {
        try {
            ...
            $this->merchant_id = PgName::PAYG.$accountId;
            $this->merchant_key_id = $formData['merchant_key_id'];
            $this->authentication_key = $formData['authentication_key'];
            $this->authentication_token = $formData['authentication_token'];
            $this->payout_account_id = $formData['payout_account_id'];
    ...
    public function validateFormData($formData, $accountId) {
        ...
        if($this->where("account_id", $accountId)->where("merchant_key_id", $accountId)->exists()) {
