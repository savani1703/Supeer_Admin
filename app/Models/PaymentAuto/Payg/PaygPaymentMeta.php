...
class PaygPaymentMeta extends Model
{
    use Encryptable;

    protected $connection = 'payment_auto';
    protected $table = 'tbl_payg_payment_meta';
    ...
    public function addMeta($formData, $accountId) {
        try {
            ...
            $this->white_listed_ip = $formData['white_listed_ip'];
            $this->payment_reason = $formData['payment_reason'];
    ...
    public function validateFormData($formData, $accountId) {
        $validator = Validator::make($formData, [
            ...
            "white_listed_ip"           => "required_if:is_seamless,1",
        ...
        if($this->where("account_id", $accountId)->where("merchant_id", $formData['merchant_id'])->where("is_seamless",  $formData['is_seamless'])->exists()) {
