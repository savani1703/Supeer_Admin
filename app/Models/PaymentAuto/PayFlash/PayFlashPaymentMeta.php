...
class PayFlashPaymentMeta extends Model
{
    use Encryptable;

    protected $connection = 'payment_auto';
    protected $table = 'tbl_payflash_payment_meta';
    ...
    public function addMeta($formData, $accountId) {
        try {
            ...
            $this->merchant_id = PgName::PAYFLASH.'_'.$formData['label'];
    ...
    public function validateFormData($formData, $accountId) {
        ...
        if($this->where("account_id", $accountId)->where("api_key", $formData['api_key'])->where("api_key", $formData['api_key'])->where("is_seamless",  $formData['is_seamless'])->exists()) {
