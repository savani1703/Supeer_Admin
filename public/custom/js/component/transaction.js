let txnPostData = {
    filter_data: {
        ...
        player_register_start_date: null,
        player_register_end_date: null,
    },
            ...
            data.forEach((item, index) => {
                htmlData += `<tr class="${item.is_blocked === '1' ? 'bg-light-danger' : ''}">
                                ...
                                <td>
                                 ...
                                 <span class="d-block "> Cust.ID:
                                 <a class="mr-1" href="/custById/${item.customer_id}"
                                           target="_blank">${item.customer_id ? item.customer_id : ""}</a>
                                 </span>
                                ...
                                <td>
                                    ...
                                    <span class="d-block"><strong class="text-muted">PG TYPE</strong>: ${item.pg_type ? item.pg_type : "-"}
                                    ${item.temp_bank_utr ? `<span class="d-block"><strong class="text-muted">Temp UTR: </strong>${item.temp_bank_utr ? item.temp_bank_utr : ""}</span>` : ''}
                                    ...
                                    ${getBlockingIndicatorForDataBlocked(item.block_data_count)}
                                    ${item.payment_data ? `<span class="d-block"><strong class="text-muted">Enter Upi : </strong>${item.payment_data ? item.payment_data : ""}</span>` : ''}
                                    ${item.success_upi_id ? `<span class="d-block"><strong class="text-muted">Success Upi : </strong>${item.success_upi_id ? item.success_upi_id : ""}</span>` : ''}
                                     <span class="d-block pt-2">${item.player_register_date ? `<strong class="text-muted">Player Register Date: </strong>`+ item.player_register_date : ""}</span>
                                     <span class="d-block">${item.player_deposit_amount ? `<strong class="text-muted">Deposit Amount : </strong>`+ item.player_deposit_amount : ""}</span>
                                     <span class="d-block">${item.player_deposit_count ? `<strong class="text-muted">Deposit Count : </strong>`+ item.player_deposit_count : ""}</span>
...
$("#txnFilerForm").on("submit", () => {
    ...
    if(txnFormData.custLevel !== "All") {
        txnPostData.filter_data.cust_level = txnFormData.custLevel;
    }
...
function generatePayinReport() {
    const txnFormData = getFormData($("#txnFilerForm"));
    let txnReportPostData = {
        filter_data: {
            merchant_order_id:null,
            merchant_id:null,
            customer_email:null,
    ...
    if(
        txnReportPostData.filter_data.hasOwnProperty(txnFormData.txtFilterKey) === true
    ) {
        txnReportPostData.filter_data[txnFormData.txtFilterKey] = txnFormData.txtFilterValue;
    }
    txnReportPostData.filter_data.status = txnFormData.txtStatus;
    if(txnFormData.daterange) {
        let splitDate = txnFormData.daterange.split(/-/);
        txnReportPostData.filter_data.start_date = moment(splitDate[0], 'DD/MM/YYYY').format('YYYY-MM-DD 00:00:00');
        txnReportPostData.filter_data.end_date = moment(splitDate[1], 'DD/MM/YYYY').format('YYYY-MM-DD 23:59:59');
    }

    FsHelper.blockUi($("#transaction_page"));
    FsClient.post("/download/report", txnReportPostData).then(response => {
        FsHelper.unblockUi($("#transaction_page"));
        toastr.success(response.message,"success",toastOption);
    }).catch( error => {
        FsHelper.unblockUi($("#transaction_page"));
...
function getBlockingIndicatorForDataBlocked(block_data_count) {
    let htmlData = "";
    if(block_data_count){
        if(block_data_count.manual_blocked_count){
            htmlData += `<div><span class="badge badge-danger" style="margin-top: 2px;">Manual Blocked : ${block_data_count.manual_blocked_count ? block_data_count.manual_blocked_count : ""}</span></div>`;
        }
        if(block_data_count.auto_blocked_count){
            htmlData += `<div><span class="badge badge-danger" style="margin-top: 2px;">Auto Blocked : ${block_data_count.auto_blocked_count ? block_data_count.auto_blocked_count : ""} </span></div>`;
        }
    }
    return htmlData;
}
